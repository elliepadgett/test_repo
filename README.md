...
- **check_displacement.py**: This program determines the maximum and minimum y-values identified across the averages of each image in each dataset. These two values are needed for accurate global scaling for all visualizations (plots, images, and gifs) produced using these datasets; otherwise, the resulting plots are auto-scaled and will not be globally aligned.
